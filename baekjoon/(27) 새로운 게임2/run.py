	...
	for turn in range(1000):
		...
		for objectNum in sorted(dataObject.keys()):
			...
			if isBlueOrOut:
				...
				nny = y + dy[dataObject[objectNum][2]]
				nnx = x + dx[dataObject[objectNum][2]]
				if not (0 <= nny <= N-1 and 0 <= nnx <= N-1):
					...
					if dataColor[nny][nnx] == 2:
						...
						# tn = dataMap[y][x].pop(objIndex)
						# dataObject[tn][0], dataObject[tn][1] = nny, nnx
						# dataMap[nny][nnx].append(tn)

						objIndex = dataMap[y][x].index(objectNum)
						tmpContainer = []
						for k in range(objIndex, len(dataMap[y][x])):
							tn = dataMap[y][x].pop(-1)
							dataObject[tn][0], dataObject[tn][1] = nny, nnx
							dataMap[nny][nnx].append(tn)
				...
				if dataColor[ny][nx] == 0: # 흰색
					...
					for k in range(objIndex, len(dataMap[y][x])):
						tn = dataMap[y][x].pop(-1)
						dataObject[tn][0], dataObject[tn][1] = ny, nx
						tmpContainer.append(tn)
					tmpContainer.reverse()
					for data in tmpContainer:
						dataMap[ny][nx].append(data)

				elif dataColor[ny][nx] == 1: # 빨강
					objIndex = dataMap[y][x].index(objectNum)
					targetIndex = len(dataMap[ny][nx])
					for k in range(objIndex, len(dataMap[y][x])):
						tn = dataMap[y][x].pop(-1)
						dataObject[tn][0], dataObject[tn][1] = ny, nx
						dataMap[ny][nx].append(tn)


			if isFinished(dataMap):
				return turn + 1
